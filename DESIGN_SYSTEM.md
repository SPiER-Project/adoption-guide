...
| Component | Owns | File |
|---|---|---|
| `SectionHeader` | the `<h3>` row: title, right-hand meta, optional title-as-toggle | [`packages/ui/src/SectionHeader.tsx`](packages/ui/src/SectionHeader.tsx) |
| `Card` | a bordered panel: `--radius-lg`, `roomy`/`compact` padding, `card`/`muted`/`brand` tone, optional brand `accent` edge | [`packages/ui/src/Card.tsx`](packages/ui/src/Card.tsx) |
| `Pill` | a small inline marker: `--radius-pill`, `sm`/`md`, `status`/`label`, tones incl. the risk ramp solid and soft | [`packages/ui/src/Pill.tsx`](packages/ui/src/Pill.tsx) |
| `Notice` | a tinted message box with a 3px left edge, six tones, `role` from tone | [`packages/ui/src/Notice.tsx`](packages/ui/src/Notice.tsx) |
| `EmptyState` | "nothing here": inline, or `panel` for an empty region | [`packages/ui/src/EmptyState.tsx`](packages/ui/src/EmptyState.tsx) |
| `DataTable` | the table shell: scroll wrapper, optional frame, header type, `compact`/`comfortable` cells, dividers; the page writes its own `<thead>`/`<tbody>` | [`packages/ui/src/DataTable.tsx`](packages/ui/src/DataTable.tsx) |
| `WorkflowForm` | the recorder frame: header, card-beside-drawer, hint, notice, code drawer | [`packages/tool-views/src/components/WorkflowForm.tsx`](packages/tool-views/src/components/WorkflowForm.tsx) |

### Risk pill — `RiskPill`

[`packages/tool-views/src/components/RiskPill.tsx`](packages/tool-views/src/components/RiskPill.tsx) is a wrapper over
...
everywhere; its colours now live in [`packages/ui/src/Pill.css`](packages/ui/src/Pill.css).
...
Runs in CI on every PR touching the app or tooling trees (see
