...
*   **`docs/plans/docs-and-ig-content-consolidation.md`**: Plan for a repo-wide narrative-consolidation cleanup, found by a review of every IG page, FSH comment, and duplicated pitch/stage-list: repo mechanics and issue numbers leaked *into* the IG pages, normative rationale leaked *out* into FSH `//` comments the publisher never renders, and the pitch/stage list were each copied 5–7 times. A sequenced set of tasks across three phases (A: make the IG pages true; B: consolidate `docs/`; C: structural changes — a narrative-truth gate, tool ids as FHIR identifiers, lifting FSH rationale into rendered pages). ⚠️ Task A0 records that the first Phase-A PR ([#468](https://github.com/SPiER-Project/adoption-guide/pull/468)) was built from a corrupted local `main` 65 commits behind `origin/main` and had to be rebased and partly redone rather than merged. Its own Status table is the source of truth for what has landed.
