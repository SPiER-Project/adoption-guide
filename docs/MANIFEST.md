...
*   **Snapshot automation (2026-08):** `.github/workflows/roadmap-snapshot.yml` — weekly cron plus `workflow_dispatch`; re-runs the fetch and opens a PR only when `{repo, issues}` actually changed (`fetchedAt` and the unrendered `updatedAt` are normalized out of the drift key). The org forbids Actions from opening PRs, so the workflow pushes `chore/roadmap-snapshot` and links a compare URL in the run summary; drop in a `ROADMAP_PR_TOKEN` secret to get real auto-PRs. **Needs `issues: read` in its `permissions:` block** — declaring that block at all resets unlisted scopes to `none`, and GITHUB_TOKEN without issues access gets HTTP 200 and an *empty array* from the issues endpoint rather than a 403. The first dispatched run hit exactly this and fetched 0 issues; the workflow's sanity check (empty / errored / >10% mass loss) is what stops such a run from PRing an emptied page.
