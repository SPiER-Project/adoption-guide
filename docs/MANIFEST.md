...
*   **Snapshot automation (2026-08):** `.github/workflows/roadmap-snapshot.yml` — weekly cron plus `workflow_dispatch`; re-runs the fetch and opens a PR only when `{repo, issues}` actually changed (`fetchedAt` and the unrendered `updatedAt` are normalized out of the drift key). The org forbids Actions from opening PRs, so the workflow pushes `chore/roadmap-snapshot` and links a compare URL in the run summary; drop in a `ROADMAP_PR_TOKEN` secret to get real auto-PRs.
