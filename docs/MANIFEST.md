...
*   **`docs/research/2026-08-us-behavioral-health-profiles-ig.md`**: Direct inspection (2026-08-12) of the HL7/ASTP **US Behavioral Health Profiles IG** CI build — what it actually contains (three profiles; the rest is unconstrained examples), the verified item-for-item C-SSRS and PHQ-9 LOINC alignment with SPiER, three structural gaps (our profiles don't require the `survey` category the federal crosswalk's named US Core profile mandates; `fallbackDispatch.ts` cannot recognize the BHP IG's own examples, and its `QuestionnaireResponse.item.code` path reads an element R4 does not define), six dated defects in the CI build, and the SAMHSA pilot cohort (45 partners, 9 states, testing completes end of 2026). Supersedes `ig/input/pagecontent/relationship-to-other-igs.md` where they conflict.
...
*   **Snapshot automation (2026-08):** `.github/workflows/roadmap-snapshot.yml` — weekly cron plus `workflow_dispatch`; re-runs the fetch and opens a PR only when `{repo, issues}` actually changed (`fetchedAt` and the unrendered `updatedAt` are normalized out of the drift key). The org forbids Actions from opening PRs, so the workflow pushes `chore/roadmap-snapshot` and — since #232 — files a reusable tracking issue with a compare link, rather than only warning into a run summary nobody opens. Either an org-owner setting (Actions → General → allow Actions to create PRs) or a `ROADMAP_PR_TOKEN` secret gets real auto-PRs; see `docs/scheduled-checks-triage.md`. **Needs `issues: read` in its `permissions:` block** — declaring that block at all resets unlisted scopes to `none`, and GITHUB_TOKEN without issues access gets HTTP 200 and an *empty array* from the issues endpoint rather than a 403. The first dispatched run hit exactly this and fetched 0 issues; the workflow's sanity check (empty / errored / >10% mass loss) is what stops such a run from PRing an emptied page.
*   **Seed script:** removed 2026-07. `scripts/seed-roadmap-issues.mjs` created the initial (2026-05) label taxonomy and tool/priority epics; its stage codes predate the SSC stage-tile rename, so re-running it would have re-created the retired `stage:flag-risk` / `stage:set-risk-status` / `stage:manage-active-risk` labels. Issues are now created in GitHub directly. See git history if the original taxonomy is ever needed.
