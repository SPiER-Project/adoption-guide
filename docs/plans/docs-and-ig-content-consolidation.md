...
| B1 — One `docs/` index; finish the archive #443 started | Sonnet 5 | half landed on `main` via #443; index merge still open |
| B2 — One home for the pitch: merge PROJECT_OVERVIEW into README | Opus 5 | not started |
| B3 — Per-tool READMEs: provenance and folder contents only | Opus 5 | not started |
| C1 — Extend `check-ig-menu.mjs` into an IG narrative gate | Opus 5 | not started; #410 built the base |
| C2 — Tool ids as `ActivityDefinition.identifier` | Fable 5.1 | not started |
| C3 — Lift FSH comment rationale into rendered IG content | Fable 5.1 | not started |
| C4 — `Overview.tsx` prose becomes a content module | Opus 5 | not started |
| C5 — File the Questionnaire-in-the-IG follow-up | Sonnet 5 | not started |
