...
requires: `npm run verify` at the repo root for anything touching the apps or the root tooling;
`npx fsh-sushi .` plus `node scripts/check-sushi-output.mjs` for anything
touching `ig/`; each Worker's own `npm run verify` (`services/guide`,
...
| Licensing evidence | `ig/input/resources/questionnaires/<tool>/licensing/MEMO.md` | the FSH extension states the *status*; the MEMO is the evidence — **keep both** |

## Standing rules for every task

- **Never hand-edit generated output**: `packages/fhir-artifacts/generated/`,
  `ig/fsh-generated/`, `docs/use-cases/dist/`, `.runtime-fhir/`.
...
- **Historical documents keep their historical wording.** `docs/plans/archive/repo-audit.md`
...
6. Every link into the app uses `#/implementation-guide/...`, which resolves
   only through `LegacyGuideRedirect` in `apps/guide/src/App.tsx`, and hardcodes
   `spier-project.github.io`. Retarget to `#/guide/<section>` (sections are
   listed in `apps/guide/src/data/guideSections.ts`) and to `#/population`. Keep the
...
For each block: the `Questionnaire.url` + `version` from `ig/input/resources/questionnaires/`, the
...
`ig/input/resources/questionnaires/` Questionnaire appears in the page; every `code=` value
...
1. `measurement.md` "What is and isn't verified" (from ~line 186) recounts the
   ...
   `packages/core/src/lib/measures.ts`, `vitest` and `npm run check:measures`. That
   history already lives in `CLAUDE.md` and
   `docs/plans/archive/stage-8-measure-and-share.md`. Reduce the section to what an
...
### A5 — Retired stage names in root README, PROJECT_OVERVIEW, ig/input/resources/questionnaires README
...
and `ig/input/resources/questionnaires/README.md:11–18` say "Flag Risk", "Set Risk Status" and
"Manage Active Risk" (the ig/input/resources/questionnaires table mixes old and new in adjacent
rows). Replace each with the CodeSystem `display`, and where a stage number is
given check it against the CodeSystem's order.

Do **not** modernize `docs/plans/archive/repo-audit.md` or `docs/plans/archive/ssc-stage-tiles-rollout.md`
...
3. `docs/plans/archive/repo-audit.md` declares itself a historical record and still sits at
...
`how-to-read.md`, `apps/guide/src/pages/Overview.tsx`, and
`public/SPiER-Overview-Care-Pathway.html`. README and PROJECT_OVERVIEW are
...
The ten `ig/input/resources/questionnaires/<tool>/README.md` files carry scoring tables, LOINC
...
`ig/input/resources/questionnaires/`), and `ig/input/resources/questionnaires/README.md`'s table matches
...
- **G. App links resolve**: every `#/<route>` must match a **non-legacy**
  route in `apps/guide/src/App.tsx` — parse `path="…"` under the `<Shell>` block and
  exclude anything rendered by `Legacy*Redirect` or `<Navigate>` — and
  `#/guide/<x>` must match `GUIDE_SECTIONS` in `apps/guide/src/data/guideSections.ts`.
...
Because G reads `web/src`, add `apps/guide/src/App.tsx` and
`apps/guide/src/data/guideSections.ts` to `ig.yml`'s path triggers — a route rename
...
3. `check:catalog` asserts: every AD the catalog wires has exactly one tool-id
   ...
   check before adding).
4. Both Workers import `@spier/core` — run `services/guide`'s and
...
Done when: `npm run verify` passes at the repo root, in `services/guide/` and
`services/mock-ehr/`,
...
`apps/guide/src/pages/Overview.tsx` holds roughly 35 paragraphs of narrative as JSX.
`apps/guide/src/data/guideSections.ts` is the repo's model for doing this right: one
ordered data file, everything derives from it. Do the same for narrative.

1. Create `apps/guide/src/content/overview.ts` (a typed array of sections: heading,
...
4. Stretch, only if cheap: emit the module's Capture/Translate/Act summary into
   `public/SPiER-Overview-Care-Pathway.html`'s build so the one-pager's
...
(include `ig/input/resources/questionnaires/` as a `path-resource` and re-test whether the NPE
