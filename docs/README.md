...
* [`plans/user-scoped-smart-launch.md`](plans/user-scoped-smart-launch.md) — how [#401](https://github.com/SPiER-Project/adoption-guide/issues/401) was closed: the caseload became a real SMART app, launched by the host with no patient in context. ⚠️ **All four phases landed (#489, #491, #494); the second half of the old title did not happen.** Phase D set out to retire the guide's bundled patient data and concluded instead that the property belongs to the `clinical` *build*, not the guide — so it pinned the boundary and filed the gate where it can be true. Read Phase D before re-proposing the retirement.
...
* [`outreach/README.md`](outreach/README.md) — the standing question lists for the dashboard author and the Zero Suicide Institute. The two-sided one-pager that used to live here was removed; that README records what went and why.
