 ...
 * ─── What it does ────────────────────────────────────────────
 *
 * Scans TypeScript under packages/core/src, the two app trees, tests/ and services/
 ...
 *
 * Usage:
 *   node scripts/check-codings.mjs                        # default server
 *   node scripts/check-codings.mjs --tx https://tx.fhir.org
 */
...
const SCAN = [
  ...
  // convention above.
  //
  // ⚠️ **Re-derived a THIRD time at the tooling hoist (2026-09-19), and the one
  // `web/src` entry became these three.** The apps/ split had already emptied
  // that tree of UI source, and the hoist deleted `web/` outright: what was
  // actually producing its loinc 2 / snomed 1 / tho 1 was the handful of
  // cross-package TEST files, which are now `tests/`. So the old floors move
  // there rather than being retired — measured, not assumed:
  //
  //   apps/guide/src     loinc  0 / snomed 0 / tho 0
  //   apps/clinical/src  loinc  2 / snomed 0 / tho 0
  //   tests              loinc  2 / snomed 1 / tho 1
  //
  // ⚠️ The old entry's floors EQUALLED its live count, so it was one deletion
  // from going red and nothing would have said so. Halved here where halving
  // leaves a floor that can still fail; a source measuring 1 keeps a floor of
  // 1, which is the smallest value that still fires when the tree stops being
  ...
  //
  // Two independent contributors sit inside packages/core/src — the runtime mappers
  ...
  // dictionary to its old un-gated shape drops that tree's LOINC from 69 to 41,
