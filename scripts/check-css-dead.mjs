...
// ⚠️ ONE PER APP, and `dirname(root.dir)` because an app's index.html is the
// SIBLING of its src/. This read `web/index.html` — a single file that the
// apps/ split had already moved out from under it, leaving the existsSync
// below false and this input silently empty. Deriving it from APP_ROOTS is
// what stops a third app from reappearing outside the scan.
const indexHtmls = APP_ROOTS.map((r) => join(dirname(r.dir), 'index.html')).filter(existsSync)
// The renderer's theme, which defines every class the renderer emits.
const vendorThemeCss = join(REPO_ROOT, 'node_modules', '@formbox', 'hs-theme', 'dist', 'index.css')
...
for (const html of indexHtmls) blob += '\n' + readFileSync(html, 'utf8')
...
console.log(
  ...
  `${indexHtmls.length > 0 ? ` + ${indexHtmls.length} index.html` : ''}; ` +
