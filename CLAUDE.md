...
- `FHIR-Resources/` — hand-authored FHIR Questionnaire JSON (plus a few CarePlan templates), one folder per instrument (ASQ, PHQ-9, C-SSRS, SBQ-R, CAMS, Stanley-Brown). Imported **directly** by `web/src/App.tsx` at runtime, **and published into the IG** through `ig/input/resources/questionnaires`, a tracked symlink to this folder named in `sushi-config.yaml`'s `path-resource` (#473). ⚠️ The publisher refuses a `path-resource` outside the IG root, which is why it is a symlink and not `../FHIR-Resources`; every resource JSON here needs an `id`, because the publisher names the page `<Type>-<id>.html` and `check-ig-narrative.mjs` fails on one without.
